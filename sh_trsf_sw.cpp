...
{
	QString name = QString::fromLocal8Bit("湿地土壤水分水位业务");
	return name;
}
		...
		if (buff[i] == 0xaa)
		{
			...
			for (int j = i + 1; j < buff.length(); j++)
			{
				if (buff[j] == 0xdd)
				{
					...
					QString fileName = QCoreApplication::applicationDirPath() + "\\SH\\TRSF_SW\\" + QString::number(frame.SrcAddr) + "\\" + current_day;
					QDir dir(fileName);
					if (!dir.exists())
...
void Transform2GPRSPerVolumeTmp(QString data, QJsonObject &json)
{
	...
	//时间格式2
	QString timeform = Convert2Time(year + month + day + hour + min + "00");
	json.insert("UploadTime", timeform);
	...
	//地下水位
	int waterlvl = data[9].unicode();
	waterlvl |= (data[10].unicode() << 8);
	waterlvl |= (data[11].unicode() << 16);
	waterlvl |= (data[12].unicode() << 24);
	json.insert("WaterLevel", (float)waterlvl/1000.000);

	//获取GPRS信号强度
	int PowerOfGPRS = data[13].unicode();
	json.insert("SignalStrength", PowerOfGPRS);

	//获取GPRS误码率
	int GPRSBER = data[14].unicode();
